...
router.post("/register", async (req, res) => {
  try {
    ...
    await newUser.save();
    const verificationLink = `https://evergreen-theta.vercel.app/verify/${newUser._id}`;
    // send verification email
    const mailOptions = {
      from: EMAIL_USER,
      to: email,
      subject: `Email Confirmation - Evergreen`,
      html: `<h1>Welcome to Evergreen</h1> Click the link below to verify your email <br> <a href="${verificationLink}">${verificationLink}</a>`
    };
...
router.post("/forgot-password", async (req, res) => {
  try {
    ...
    const resetLink = `https://evergreen-theta.vercel.app/reset-password?token=${resetToken}`;
...
router.get("/property-agent", (req, res) => {
  res.sendFile(path.join(__dirname, "..", "public", "p-agent.html"));
});
...
// Serve as menu page
router.get("/property-listing", (req, res) => {
  res.sendFile(path.join(__dirname, "..", "public", "p-list.html"));
});
